...
const TIPOS_RELATORIO = [
  ...
  { id: "pei", label: "PEI — Plano Educacional Individualizado", descricao: "Metas e estratégias para a equipe multidisciplinar", emoji: "📋", cor: "#BA7517" },
  { id: "paee", label: "PAEE — Plano de AEE", descricao: "Plano de Atendimento Educacional Especializado", emoji: "📁", cor: "#0F6E56" }
];

// Todas as opções de período letivo — bimestres, trimestres, semestres e anual
const PERIODOS = [
...
export default function Reports() {
  ...
  function gerarTextoRelatorio(r) {
    ...
    if (rep.areas_de_atencao?.length) { txt += `ÁREAS DE ATENÇÃO:\n`; rep.areas_de_atencao.forEach(a => { txt += `  • ${a}\n`; }); txt += "\n"; }
      ...
      <header style={{
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {relatorio && (
            ...
            }}>⬇️ Baixar relatório</button>
          )}
          <button onClick={() => navigate("/relatorios/salvos")} style={{
            fontSize: 13, padding: "8px 16px", background: "#fff",
            color: "#0F6E56", border: "0.5px solid #0F6E56",
        ...
        {!relatorio ? (
          <div style={{ background: "#fff", border: "0.5px solid #d3d1c7", borderRadius: 12, padding: "1.5rem", boxShadow: "0 2px 8px rgba(43,158,195,0.06)" }}>
            ...
            <div style={{ marginBottom: 20 }}>
              ...
              <select
                ...
                onChange={e => { const a = alunos.find(a => a.id === e.target.value); setAlunoSelecionado(a || null); }}
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
                {TIPOS_RELATORIO.map(tipo => (
                  <div key={tipo.id} onClick={() => setTipoSelecionado(tipo.id)} style={{
                    ...
                    <p style={{ fontSize: 12, color: "#5f5e5a", margin: 0 }}>{tipo.descricao}</p>
                  </div>
                ))}
              </div>
            </div>

            {/* Período letivo — agora como select com todas as opções */}
          ...
          <div>
            <button onClick={() => setRelatorio(null)} style={{
              fontSize: 13, marginBottom: 20, color: "#2B9EC3",
            ...
            {/* Cabeçalho */}
            <div style={{ background: "linear-gradient(135deg, #2B9EC3, #4CAF82)", borderRadius: 12, padding: "1.5rem", marginBottom: 20, color: "#fff" }}>
            ...
            {relatorio.metrics && (
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  <div key={i} style={{ background: "#fff", border: "0.5px solid #d3d1c7", borderRadius: 10, padding: "14px", textAlign: "center" }}>
            ...
            {relatorio.report && (
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                ...
                {relatorio.report.pontos_positivos?.length > 0 && (
                  <Secao titulo="Pontos positivos" cor="#4CAF82">
                    {relatorio.report.pontos_positivos.map((p, i) => (
                      <div key={i} style={{ background: "#edfff6", border: "0.5px solid #4CAF82", borderRadius: 6, padding: "8px 12px", marginBottom: 8, fontSize: 14 }}>✅ {p}</div>
                    ))}
                  </Secao>
                )}
                {relatorio.report.areas_de_atencao?.length > 0 && (
                  <Secao titulo="Áreas de atenção" cor="#BA7517">
                    {relatorio.report.areas_de_atencao.map((a, i) => (
                      <div key={i} style={{ background: "#faeeda", border: "0.5px solid #BA7517", borderRadius: 6, padding: "8px 12px", marginBottom: 8, fontSize: 14 }}>⚠️ {a}</div>
                    ))}
                  </Secao>
                )}
                {relatorio.report.recomendacoes?.length > 0 && (
                  <Secao titulo="Recomendações pedagógicas" cor="#2B9EC3">
                    {relatorio.report.recomendacoes.map((r, i) => (
                      <div key={i} style={{ background: "#e8f7fd", border: "0.5px solid #2B9EC3", borderRadius: 6, padding: "8px 12px", marginBottom: 8, fontSize: 14 }}>💡 {r}</div>
                    ))}
                  </Secao>
                )}
                {relatorio.report.metas_proxima_periodo?.length > 0 && (
                  <Secao titulo="Metas para o próximo período" cor="#534AB7">
                    {relatorio.report.metas_proxima_periodo.map((m, i) => (
                      <div key={i} style={{ background: "#EEEDFE", border: "0.5px solid #534AB7", borderRadius: 6, padding: "8px 12px", marginBottom: 8, fontSize: 14 }}>🎯 {m}</div>
                    ))}
                  </Secao>
                )}
                ...
                {relatorio.report.base_legal && (
                  <div style={{ background: "#f1efe8", border: "0.5px solid #d3d1c7", borderRadius: 8, padding: "12px 16px", fontSize: 12, color: "#5f5e5a" }}>
...
function Secao({ titulo, cor, children }) {
  return (
    <div style={{ background: "#fff", border: "0.5px solid #d3d1c7", borderLeft: `3px solid ${cor}`, borderRadius: 10, padding: "16px 20px" }}>
