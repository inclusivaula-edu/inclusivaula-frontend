...
export default function Attendance() {
  ...
  async function handleSalvar() {
    if (Object.keys(presencas).length === 0) {
      mostrarFeedback("Registre pelo menos uma presença ou falta.", "erro");
      return;
    }
    setSalvando(true);
    try {
  ...
  function calcularFrequencia(alunoId) {
    ...
    return {
      ...
      pct: Math.round((presentes / registros.length) * 100)
    };
  }
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8, marginBottom: 20 }}>
                  {alunos.map(a => (
                    <div key={a.id} style={{
                      ...
                      background: presencas[a.id] === true ? "#edfff6"
                        : presencas[a.id] === false ? "#fcebeb" : "#f5f9ff",
                      border: `0.5px solid ${presencas[a.id] === true ? "#4CAF82"
                        : presencas[a.id] === false ? "#a32d2d" : "#d3d1c7"}`
                    }}>
                      <div>
